...
- `nader` update
  - Financial rewards for DAO contributors
    - Met with Jess https://twitter.com/thattallguy
      - Member of SeedClub https://twitter.com/seedclubhq
        - DAO for creating liquidity for DAO members
        - SeedClub has seed rounds where they help create a game plan for things
          - How to create tokens and reward users
          - How to generate and distribute liquidity
            - Bring in outside VCs to help provide liquidity
          - Setting up governance
        - `codingwithmanny`: How does SeedClub work?
          - `ERC-20` tokens that community members hold
          - `$FWB` treasury allots tokens that provide value but don't provide
            responsibility, voting rights, etc.
            - Separate token for that thing
          - SeedClub is essentially a group of experts that can help us figure
            out where we're going and how to
          - `codingwithmanny`: What does SeedClub get out of it?
            - They receive tokens from the DAOs they help
              - Establishes equity for them by investing in many different DAOs
    ...
    - Create a setup whereby if real value is created, those who contribute to
      creating that value should ideally be able to realise that value in the
      form of liquid ownership via a token.
